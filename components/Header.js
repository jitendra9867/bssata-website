...
export default function Header() {
  ...
  return (
    <header className="boxed-header">
      {/* Single row header: Logo left, nav + calendar right */}
      <div className="bg-[#8B2500]">
        <div className="boxed-nav-container">
          <div className="flex items-center justify-between gap-4 py-[10px]">
            ...
            <div className="hidden lg:flex items-center gap-1.5">
              <nav className="flex items-center gap-0.5" ref={dropdownRef}>
                {navLinks.map((link) => (
                  ...
                  >
                    <Link
                      href={link.href}
                      className={`relative px-2 xl:px-2.5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center gap-1 ${
              ...
              {/* Calendar button - direct PDF download */}
              <a
                href="/images/calendars/cal%202026.pdf"
                download
                className="inline-flex items-center gap-2 px-5 py-2 rounded-full text-[13px] font-bold text-white bg-[#D60C0C] hover:bg-[#C20B0B] transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 flex-shrink-0"
              >
      ...
      <div className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${mobileOpen ? 'max-h-[80vh] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="bg-white border-t border-cream-200/60 shadow-xl max-h-[80vh] overflow-y-auto">
          <div className="boxed-nav-container py-3 space-y-0.5">
            ...
            {/* Mobile calendar button - direct PDF download */}
